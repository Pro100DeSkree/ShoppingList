package com.deskree.shoppinglist.billing
            ...
            if(!purchase.isAcknowledged){
                ...
                bClient?.acknowledgePurchase(acParams){
                    if(it.responseCode == BillingClient.BillingResponseCode.OK){
                        savePref(true)
                        Toast.makeText(activity, "Дякую за покупку!", Toast.LENGTH_LONG).show()
                    } else {
                        savePref(false)
                        Toast.makeText(activity, "Не вдалось зробити купівлю", Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
